=== WooCommerce Quote Product ===
Contributors: (this should be a list of wordpress.org userid's)
Tags: woocommerce, product, ecommerce
Requires at least: 4.2.2
Tested up to: 4.2.2
Stable tag: 4.2.2
License: GPLv2 or later License
URI: http://www.gnu.org/licenses/gpl-2.0.html
WC requires at least: 2.3.9
WC tested up to: 2.3.9

== Description ==

WooCommerce Extension Plugin - Add an option to product to be "call for quote" product, and disable online ordering

= Notes =
*If you are theme is already overwriting the following woocommerce template, you need to reoverwrite the templates using the file
located under this plugin's woocommerce folder.

== Installation ==


== Frequently Asked Questions ==



== Screenshots ==



== Changelog ==

= 0.1 =
*Alpha - first release
*The quote product text in the single product page is defined in $quote_product_text  (line 15 of woocommerce-quote-product.php)
*Works with simple product only
*Known Bug - not compatible with WooCommerce Product AddOn (The products with AddOn fields added)